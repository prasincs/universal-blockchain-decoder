...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  ...
  `cargo update` moved `alloy-consensus`/`alloy-eips` 2.2.0 -> 2.4.1 and
  ...
  No findings. `upstream_outdated` alloy entries cleared.
  **Incidental fix**: a newer clippy (rust-1.96.0) flagged pre-existing
  `unnecessary_unwrap` in `decoder-crypto-zk/tests/ecdsa_tests.rs:157`
  (`assert_eq!(result1.unwrap(), result2.unwrap())` after an `is_ok()` guard),
  breaking the mandatory `cargo clippy -D warnings` gate for the whole
  workspace. Rewrote the guard to `if let (Ok(v1), Ok(v2)) = (&result1,
  &result2)` — same test semantics, clippy clean.)
- [ ] *(auto-generated 2026-08)* **Bump solana-transaction-status 4.1.2 ->
  ...
  findings. Verify: `cargo test -p decoder-solana`; ratchet `upstream_outdated`
  drops.
