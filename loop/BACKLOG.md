...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  ...
  `alloy-tx-macros` 2.2.0 -> 2.4.1 in Cargo.lock. `alloy-primitives` stays at
  1.6.1 (separately versioned, never in `upstream_outdated`). No API migration
  ...
  recovered sender). No findings. `upstream_outdated` 7 -> 5.)
- [ ] *(auto-generated 2026-08)* **Bump solana-transaction-status 4.1.2 ->
  ...
  findings (minimal repro fixture + backlog entry).
  Verify: `cargo test -p decoder-solana`.
- [ ] *(auto-generated 2026-08)* **Bump alloy-rlp 0.3.15 -> 0.3.16** in the
  locked graph (no dedicated differential test; patch bump). Fold into the
  next `cargo update` gated by the full suite.
