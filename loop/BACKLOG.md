...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.3.0** in
  ...
  still agree field-for-field (types, chain_id, nonce, gas, to, value, input,
  access list, v/r/s, tx hash, recovered sender). No findings.
  `upstream_outdated` 6 -> 4.)
  **Finding (unrelated, pre-existing on HEAD, fixed here to keep the clippy
  gate green)**: a toolchain drift to clippy rust-1.96.0 now rejects
  `ecdsa_tests.rs:156-158`'s `is_ok()`-then-`unwrap()` pattern
  (`unnecessary_unwrap`, `-D warnings`), which blocked every commit. Rewrote
  it as `if let (Ok(v1), Ok(v2)) = (result1, result2)` — behavior-preserving.
