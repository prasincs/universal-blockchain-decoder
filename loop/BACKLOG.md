...
- [x] *(auto-generated 2026-09)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  decoder-ethereum dev-deps. (Done 2026-09; `cargo update` moved
  `alloy-consensus`/`alloy-eips` 2.2.0 -> 2.4.1, `alloy-rlp` 0.3.15 -> 0.3.16
  (+ `alloy-serde`/`alloy-tx-macros`/`alloy-rlp-derive`), and
  `alloy-primitives` 1.6.1 -> 1.7.1 in Cargo.lock. No API migration needed —
  ...
  recovered sender). No findings. `upstream_outdated` 8 -> 4.
  Incidentally fixed the pre-existing clippy blocker at
  `decoder-crypto-zk/tests/ecdsa_tests.rs:157` (`unnecessary_unwrap` under
  clippy 1.96 — rewrote as a `match`, assertion unchanged) so the bump could
  pass the mandatory clippy gate; see the "Workspace fails clippy" item.)
- [ ] *(auto-generated 2026-09)* **Bump solana-transaction-status 4.1.2 ->
  4.2.2** in decoder-solana dev-deps; re-run
  `solana_transaction_status_differential`. Acceptance: `cargo update -p
  solana-transaction-status --precise 4.2.2`; the differential test still
  agrees field-for-field; `upstream_outdated` shrinks.
...
- [ ] **Workspace fails clippy on current stable** — a moving target as the
  ...
  a `match` or destructure, do NOT weaken the assertion). **Code half DONE
  2026-09**: rewrote `ecdsa_tests.rs:145-164` as a `match` (assertion
  unchanged); `cargo clippy --all --all-targets --all-features -- -D warnings`
  is now clean on clippy 1.96. Remaining: pin/refresh the CI toolchain so
  local and CI clippy agree. The earlier
