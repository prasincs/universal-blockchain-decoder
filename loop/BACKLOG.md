...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.3.0** in
  ...
  `alloy-eips`/`alloy-serde`/`alloy-tx-macros` 2.2.0 -> 2.3.0 in Cargo.lock.
  Minor bump, no API migration needed — all 7 `alloy_differential` tests still
  agree field-for-field (types, chain_id, nonce, gas, to, value, input, access
...
- [ ] *(auto-generated 2026-08)* **Bump solana-transaction-status 4.1.2 ->
  ...
  `solana_transaction_status_differential`. Real differential test. Minor bump
  within the "4.1" range (needs `--precise`). Verify: lock moves to 4.2.0 and
  both differential tests still agree field-for-field.
- [ ] *(auto-generated 2026-08)* **Bump alloy-rlp 0.3.15 -> 0.3.16** — patch
  bump. `alloy-rlp` is a `decoder-bnb` dev-dep with no real differential test
  yet (dead validation dep); low value alone, fold into the BNB differential
  work or the re-pin cadence sweep.
