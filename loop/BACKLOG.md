...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  ...
  `alloy-tx-macros` 2.2.0 -> 2.4.1 in Cargo.lock (`alloy-primitives` unchanged
  at 1.6.1). No API migration needed — all 7 `alloy_differential` tests still
  agree field-for-field (type, chain_id, nonce, gas, to, value, input, access
...
- [ ] *(auto-generated 2026-08)* **Bump solana-transaction-status 4.1.2 ->
  ...
  Verify: `cargo update -p solana-transaction-status` moves the lock to 4.2.1;
  `cargo test -p decoder-solana` passes.
