...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  decoder-ethereum dev-deps. (Done 2026-08; `cargo update -p alloy-consensus
  -p alloy-eips` moved `alloy-consensus`/`alloy-eips`/`alloy-serde`/
  `alloy-tx-macros` 2.2.0 -> 2.4.1 in Cargo.lock. No API migration needed —
...
- [x] **TON `real_transactions` integration tests fail on main** — root
  ...
  covers core.
- [x] **Workspace fails clippy on current stable** (Done 2026-08; rewrote
  `decoder-crypto-zk/tests/ecdsa_tests.rs:156-158` from
  `result1.unwrap()`/`result2.unwrap()` after an `is_ok()` check to an
  `if let (Ok(v1), Ok(v2)) = (result1, result2)` destructure — same
  determinism assertion, no `unnecessary_unwrap`. `cargo clippy --all
  --all-targets --all-features -- -D warnings` now exits 0 under clippy
  1.96.0.) Remaining: pin/refresh the CI toolchain so local and CI clippy
  agree, and re-open if a newer clippy flags something else — a moving target
  as the toolchain advances and CI's toolchain lags. Under clippy **1.96.0**
