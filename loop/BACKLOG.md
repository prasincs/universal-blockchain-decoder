...
- [x] *(auto-generated 2026-07)* **Bump solana-transaction-status 3.1 -> 4.x**
  ...
  `solana_transaction_status_differential`. (Done 2026-07; bumped 3.1.14 ->
  4.1.2. v4 gates all public types behind the `agave-unstable-api` feature, so
  the dev-dep now enables it; `EncodedTransaction`/`TransactionBinaryEncoding`
  moved to root re-exports from `solana-transaction-status-client-types` but
  `EncodedTransaction::decode()` is unchanged. Both differential tests still
