...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  decoder-ethereum dev-deps (`alloy-consensus`/`alloy-eips`); re-run
  `alloy_differential`. (Done 2026-08; `cargo update -p alloy-consensus/-p
  alloy-eips --precise 2.4.1` moved `alloy-consensus`/`alloy-eips`/`alloy-serde`/
  `alloy-tx-macros` 2.2.0 -> 2.4.1 in Cargo.lock. No API migration needed — all
