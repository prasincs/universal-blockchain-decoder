...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  ...
  access list, v/r/s, tx hash, recovered sender). No findings.
  `upstream_outdated` 7 -> 5.)
  **Incidental fix**: toolchain moved to clippy 1.96.0, whose new
  `unnecessary_unwrap` lint failed the `-D warnings` gate on the pre-existing
  `decoder-crypto-zk/tests/ecdsa_tests.rs:157` (`is_ok()`-guarded `unwrap`).
  Rewrote as `if let (Ok(a), Ok(b)) = (&result1, &result2)` — behavior
  identical, all 23 `ecdsa_tests` still pass.
- [ ] *(auto-generated 2026-08)* **Bump solana-transaction-status 4.1.2 ->
  ...
  findings (minimal repro fixture + entry first).
  Verify: `cargo test -p decoder-solana` and `upstream_outdated` no longer
  lists `solana-transaction-status`.
