...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  ...
  `alloy-tx-macros` 2.2.0 -> 2.4.1 and `alloy-primitives` 1.6.1 -> 1.7.1 in
  Cargo.lock. No API migration needed — all 7 `alloy_differential` tests still
  agree field-for-field (types, chain_id, nonce, gas, to, value, input, access
  list, v/r/s, tx hash, recovered sender). No findings.
  `upstream_outdated` 8 -> 5.)
