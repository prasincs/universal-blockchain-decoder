...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.3.0** in
  ...
  `cargo update -p alloy-consensus -p alloy-eips` moved
  `alloy-consensus`/`alloy-eips`/`alloy-serde`/`alloy-tx-macros` 2.2.0 -> 2.3.0
  in Cargo.lock. No API migration needed — all 7 `alloy_differential` tests
  still agree field-for-field. No findings. `upstream_outdated` 7 -> 5.
  Incidental: fixed a pre-existing `clippy::unnecessary_unwrap` in
  `decoder-crypto-zk/tests/ecdsa_tests.rs` that a newer clippy toolchain now
  denies, which was blocking the loop's own clippy gate.)
- [ ] *(auto-generated 2026-08)* **Bump solana-transaction-status 4.1.2 -> 4.2.0**
  in decoder-solana dev-deps; re-run
  `solana_transaction_status_differential`. Acceptance:
  `cargo update -p solana-transaction-status` moves it to 4.2.0 in Cargo.lock
  and `cargo test -p decoder-solana --test solana_transaction_status_differential`
  passes.
