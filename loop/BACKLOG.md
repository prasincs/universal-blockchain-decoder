...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.4.1** in
  ...
  `cargo update` moved `alloy-consensus`/`alloy-eips` 2.2.0 -> 2.4.1,
  `alloy-primitives` 1.6.1 -> 1.7.1 (and `alloy-serde`/`alloy-tx-macros`
  2.2.0 -> 2.4.1) in Cargo.lock. No API migration needed — all 7
  `alloy_differential` tests still agree field-for-field (type, chain_id,
...
- [x] **Workspace fails clippy on current stable** — a moving target as the
  ...
  `result1.unwrap()`/`result2.unwrap()` after an `is_ok()` check).
  (Fixed 2026-08; rewrote the `is_ok()` guard + `.unwrap()` pair as an
  `if let (Ok(v1), Ok(v2)) = (&result1, &result2)` destructure — assertion
  unchanged. `cargo clippy --all --all-targets --all-features -- -D warnings`
  is green. CI toolchain pin still open below.) The earlier
