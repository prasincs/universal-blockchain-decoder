...
- [x] *(auto-generated 2026-08)* **Bump alloy oracle 2.2.0 -> 2.3.0** in
  ...
  `cargo update -p alloy-consensus/-p alloy-eips --precise 2.3.0` moved
  `alloy-consensus`/`alloy-eips` (+ transitive `alloy-serde`/`alloy-tx-macros`)
  2.2.0 -> 2.3.0 in Cargo.lock. `alloy-primitives` stays 1.6.1 (separately
  versioned, never in `upstream_outdated`). No API migration needed — all 7
