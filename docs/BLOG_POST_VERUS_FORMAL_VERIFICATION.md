# The $MILLIONS Unwrap(): How Formal Verification Could Have Prevented Cloudflare's 3-Hour Outage

**TL;DR**: On November 18, 2025, a single `.unwrap()` call caused a 3-hour global Cloudflare outage affecting millions of users. Formal verification tools like Verus could have caught this at compile time—before it reached production. This post explores how mathematical proof can prevent entire classes of bugs, with working examples you can run today.
...
**What went wrong:**
1. `parse_feature_file()` returned `Result<Vec<Feature>, ParseError>`
2. Developer used `.unwrap()` expecting file to always be valid
3. File became corrupted during deployment
4. `.unwrap()` panicked instead of returning `Err`
5. Panic propagated, crashed critical services
6. Global outage: 3 hours, millions of users affected
...
**Traditional testing** checks if your code works for specific inputs:
```rust
#[test]
fn test_parse_valid_file() {
    let result = parse_feature_file("valid.txt");
    assert!(result.is_ok());
}
```
✅ Proves: Works for `valid.txt`
❌ Doesn't prove: Works for **all** files

**Formal verification** uses mathematics to prove properties hold for **all possible inputs**:
```rust
#[requires(bytes.len() > 0)]  // Precondition
fn first_byte(bytes: &[u8]) -> u8 {
    bytes[0]  // Safe: proof guarantees len > 0
}
```
✅ Proves: Cannot panic for **any** input
✅ Compiler rejects: Calls without precondition
...
## Real Example: Cloudflare Bug Under Verus

Let's see what happens when we verify the Cloudflare code with Verus.
...
1. **Parsing untrusted input**
   ...
   - Example: RLP decoding, Bitcoin transaction parsing
...
```
Is this code safety-critical or high-leverage?
├─ No → Skip Verus (use tests)
└─ Yes → Can bugs cause outages or financial loss?
    ├─ No → Skip Verus (use tests)
    └─ Yes → Is the codebase stable?
        ├─ No → Wait, use tests for now
        └─ Yes → Use Verus
```
...
**Early evidence:**
- ChatGPT can write Verus annotations (with guidance)
...
- Verification adds cognitive overhead (not everyone needs it)
- Opt-in like clippy/miri, not mandatory
- Verus is to `rustc` what `clippy` is to type checking

Rust gives you the **foundation** (memory safety). Verus builds **the next floor** (panic-freedom).

### 2. "This is cherry-picking—one outage doesn't prove anything"
...
### 3. "Formal verification slows down development"
...
### 4. "Too immature / unproven"

**Counterevidence:**
- **seL4:** 15 years in production (verified kernel, zero bugs)*
- **IronFleet:** Deployed at Microsoft (2015)
- **Amazon:** Uses TLA+ for 10+ years
- **Verus:** 2 OSDI Best Papers (2024)

*seL4's verified kernel has had zero vulnerabilities; the ecosystem has had bugs (see CVE-2020-28928).
...
4. **Install VSCode extension**: [verus-analyzer](https://marketplace.visualstudio.com/items?itemName=verus-lang.verus-analyzer)
...
- **Martin Kleppmann** for thought leadership on formal methods
- The Rust community for building memory-safe foundations

---

**About the Author**: This post emerged from exploring formal verification for the [Universal Blockchain Decoder](https://github.com/prasincs/universal-blockchain-decoder) project, where panic-freedom in transaction parsing is critical.
...
**Corrections?** Open an issue or PR on the [examples repo](https://github.com/prasincs/formal-verification-experiments).
