...
fn test_minimal_transfer_transaction() {
    ...
    tx_bytes.extend_from_slice(&[
        ...
        0x32, 0x10, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10, 0xfe, 0xdc, 0xba, 0x98, 0x76,
        0x54, 0x32, 0x10, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10, 0xfe, 0xdc, 0xba, 0x98,
        0x76, 0x54, 0x32, 0x10,
    ]);
    ...
    tx_bytes.extend_from_slice(&[
        0x9f, 0xe5, 0xa3, 0x9b, 0x7e, 0x4f, 0x4c, 0x8d, 0x1c, 0x2e, 0x3a, 0x5b, 0x6c, 0x7d, 0x8e,
        0x9f, 0xa0, 0xb1, 0xc2, 0xd3, 0xe4, 0xf5, 0x06, 0x17, 0x28, 0x39, 0x4a, 0x5b, 0x6c, 0x7d,
        0x8e, 0x9f,
    ]);
    ...
    let decoded = SolanaDecoder::decode(&tx_bytes).expect("Failed to decode with our parser");
...
fn test_decode_and_canonicalize() {
    ...
    // Decode
    let decoded = SolanaDecoder::decode(&tx_bytes).expect("Failed to decode");
...
fn test_compact_u16_edge_cases() {
    ...
    for (bytes, expected) in test_cases {
        ...
        let result = read_compact_u16(&mut cursor).unwrap_or_else(|_| {
            panic!(
                "Failed to decode compact-u16 for expected value {}",
                expected
            )
        });
...
fn test_invalid_instruction_indices() {
    ...
    assert!(
        result.is_err(),
        "Should reject instruction with invalid program_id_index"
    );
}
...
fn test_multi_instruction_transaction() {
    ...
    let decoded =
        SolanaDecoder::decode(&tx_bytes).expect("Failed to decode multi-instruction transaction");
