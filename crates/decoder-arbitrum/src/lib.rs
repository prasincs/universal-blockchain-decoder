...
//!
//! This module provides a decoder for Arbitrum transactions, transforming them
//! from their native RLP format into the universal TxIR representation.
//!
//! ## Implementation Strategy
//!
//! Arbitrum is EVM-compatible and uses the **exact same transaction format as Ethereum**.
//! This decoder **reuses the Ethereum decoder** with Arbitrum-specific chain ID validation.
//!
//! ## Transaction Format
//!
...
///
/// **Reuses Ethereum decoder** with Arbitrum-specific chain ID validation.
...
impl ChainDecoder for ArbitrumDecoder {
    ...
    fn decode(raw_bytes: &[u8]) -> Result<Self::TxSpecific> {
        ...
        // Validate chain ID is for Arbitrum
        if let Some(chain_id) = tx.chain_id {
            if chain_id != 42161 {
                return Err(DecoderError::invalid_structure(
                    format!("Invalid Arbitrum chain ID: {} (expected 42161)", chain_id)
                ));
            }
        }
...
mod tests {
    ...
    fn test_validate_format() {
        ...
        assert!(ArbitrumDecoder::validate_format(&[0x01, 0x02, 0x03, 0x04]).is_err());

        // Valid minimum length should pass basic validation
        let dummy_tx = vec![0xf8, 0x6c, 0x00, 0x00, 0x00];
        assert!(ArbitrumDecoder::validate_format(&dummy_tx).is_ok());
    }

    #[test]
    fn test_decoder_reuses_ethereum() {
        // Verify that ArbitrumDecoder uses EthereumTransaction type
        use std::any::TypeId;
        ...
        type ArbitrumTxType = <ArbitrumDecoder as ChainDecoder>::TxSpecific;
        assert_same_type::<ArbitrumTxType, EthereumTransaction>();
    }
}
