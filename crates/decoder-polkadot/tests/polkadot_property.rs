//! Property-based tests for Polkadot decoder
//!
...
proptest! {
    ...
    fn test_decoder_never_panics(bytes in prop::collection::vec(any::<u8>(), 0..1000)) {
        let _result = PolkadotDecoder::decode(&bytes);
        // Should never panic, only return errors
    }

    #[test]
    fn test_validate_format_consistency(bytes in prop::collection::vec(any::<u8>(), 0..1000)) {
        let validate_result = PolkadotDecoder::validate_format(&bytes);
        let decode_result = PolkadotDecoder::decode(&bytes);
    ...
    fn test_hash_determinism(bytes in prop::collection::vec(any::<u8>(), 4..1000)) {
        ...
        if bytes.len() >= 4 {
            let hash1 = PolkadotTransaction::calculate_hash(&bytes);
            let hash2 = PolkadotTransaction::calculate_hash(&bytes);
    ...
    fn test_canonicalize_never_panics_on_valid_tx(
        ...
        if let Ok(tx) = PolkadotDecoder::decode(&with_length) {
    ...
    #[test]
    fn test_registry_chain_lookup_consistent(chain_id in 0u32..10000) {
        ...
        if let (Some(c1), Some(c2)) = (result1, result2) {
            prop_assert_eq!(c1.chain_id, c2.chain_id);
    ...
    fn test_blake2_hash_collision_resistance(
        ...
        if bytes1 != bytes2 {
            let hash1 = PolkadotTransaction::calculate_hash(&bytes1);
            let hash2 = PolkadotTransaction::calculate_hash(&bytes2);
...
fn test_decode_canonicalize_determinism() {
    ...
    extrinsic.push(0x04); // Pallet
    extrinsic.push(0x00); // Call
    ...
    let tx1 = PolkadotDecoder::decode(&with_length).unwrap();
    let tx2 = PolkadotDecoder::decode(&with_length).unwrap();
