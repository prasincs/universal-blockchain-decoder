...
fn build_state_deltas(_messages: &[CosmosMessage]) -> Result<StateDeltas> {
    // Cosmos is an account chain: there are no byte-derivable UTXO in/out
    // facts, and balance effects are NOT computable from transaction bytes.
    // This function previously fabricated `balance_change: +/-1` direction
    // sentinels and abused `outputs` for IBC/WASM messages; both were
    // removed (docs/CONCEPTS_REVIEW.md C1).
    Ok(StateDeltas {
        inputs: Vec::new(),
        outputs: Vec::new(),
    })
