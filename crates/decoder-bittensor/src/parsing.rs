...
//!
//! SCALE is the encoding format used by Bittensor and all Substrate-based chains.
...
/// Parse a SCALE-encoded Bittensor extrinsic
...
fn parse_address(bytes: &[u8], offset: &mut usize) -> Result<BittensorAddress> {
...
fn parse_signature(bytes: &[u8], offset: &mut usize) -> Result<BittensorSignature> {
    ...
    match sig_type {
        0x00 => {
            ...
            Ok(BittensorSignature::Ed25519(sig))
        }
        0x01 => {
            // Sr25519: 64 bytes
            let sig = read_bytes(bytes, offset, 64)?;
            Ok(BittensorSignature::Sr25519(sig))
        }
        0x02 => {
            // ECDSA: 65 bytes
            let sig = read_bytes(bytes, offset, 65)?;
            Ok(BittensorSignature::Ecdsa(sig))
        }
        _ => Err(DecoderError::invalid_structure(format!(
